...
var getSchedules = function(line, stop, cb) {
  request.get('http://wap.strav.mobi/horaireligne.php?ligne=' + line + '&arret=' + stop, function(err, res, body) {
    ...
    if (schedules.length === 0) {
      ...
      } else if (body.search(lines.STRING_UNKNOWN_LINE) > -1) {
        return cb(new Error('The line does not exist'), null);
      } else if (body.search(lines.STRING_UNKNOWN_STOP) > -1) {
        return cb(new Error('The stop does not exist'), null);
      } else {
        return cb(new ServerError('Server did not sent any schedules'), null);
      }
    } else {
